	...
	Scrollable
{
    ...
    // mjt 2007-5-13 paintGraph
    // paintGraph provides an easy way to access the paintSubgraph functionality,
    // bypassing the scaling in componentPaint.
    public void paintGraph (Graphics2D g2d, Shape clipper) {
    	nodeLabels = true;
    	subgLabels = true;
	    edgeLabels = true;

	    Color bkgdColor = (Color)(graph.getGrappaAttributeValue(GRAPPA_BACKGROUND_COLOR_ATTR));
	    g2d.setPaint(bkgdColor);
	    g2d.fill(clipper);
	    
    	paintSubgraph ((Graphics2D)g2d, subgraph, clipper, bkgdColor);
    }
	...
	if(bbox != null && grappaNexus != null && subg.visible && !grappaNexus.style.invis && clipper.intersects(bbox)) {
		...
		if(subg.isCluster() || Grappa.outlineSubgraphs) {
		    if(GrappaStyle.defaultStroke != grappaNexus.style.stroke) {
		        ...
		        // mjt 2007-6-20 use bounding box info from attribute, not nexus
		        // (bb info in nexus seems to randomly set min x to zero and extend boxes up to the left margin.  
		        // System.out.println(subg.getAttribute("bb"));
		        // System.out.println(subg.grappaNexus.shape);
		        // grappaNexus.draw(g2d);
		        g2d.draw((Rectangle2D)(subg.getAttributeValue("bb")));
